...
export const sendMail = async(email: string, type:string)=>{
    ...
    if(type == 'reset'){
        ...
        let button = `<a href=${BASE_URL}/authenticate/?email=${encode}> Reset Password</a>`
...
export const sendFunMail = async(email: string, name:string, type:string)=>{
    ...
    if(type == 'create'){
        ...
        let button = `<a href=${BASE_URL}/authenticate/?email=${encodeEmail}&name=${encodeName}> Create Account</a>`
